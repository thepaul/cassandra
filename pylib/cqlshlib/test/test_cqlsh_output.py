...
class TestCqlshOutput(BaseTestCase):
    ...
    def assertQueriesGiveColoredOutput(self, queries_and_expected_outputs,
        ...
        with testrun_cqlsh(tty=True, **kwargs) as c:
            for query, expected in queries_and_expected_outputs:
                ...
                output = c.cmd_and_response(query)
    ...
    def test_no_prompt_or_colors_output(self):
        ...
        queries = (('select * from has_all_types limit 1;', 4),
                   ('select * from has_value_encoding_errors limit 1;', 5))
        ...
        for termname in ('', 'dumb', 'vt100', 'xterm'):
            ...
            for cql, lines_expected in queries:
                ...
                self.assertEqual(len(output), lines_expected,
                                 msg='output: %r' % '\n'.join(output))
                self.assertNicelyFormattedTableHeader(output[0])
                self.assertNicelyFormattedTableRule(output[1])
                self.assertNicelyFormattedTableData(output[2])
                self.assertEqual(output[3].strip(), '')
    ...
    def test_numeric_output(self):
        self.assertCqlverQueriesGiveColoredOutput((
            ...
            ------------------+-----------+----------
                  ...
                  19952.11882 |         1 |     -2.1
             GGGGGGGGGGGGGGGG      GGGGGG      GGGGG
                        1E-14 |     1e+07 |    1e+05
             GGGGGGGGGGGGGGGG      GGGGGG      GGGGG
                          0.0 |         0 |        0
             GGGGGGGGGGGGGGGG      GGGGGG      GGGGG
             10.0000000000000 |    -1e+03 |    1e+08
             GGGGGGGGGGGGGGGG      GGGGGG      GGGGG
    ...
    def check_describe_keyspace_output(self, output, qksname):
        for expected_bit in (
                r'(?im)^CREATE KEYSPACE %s WITH\b' % re.escape(qksname),
                r'\bstrategy_class =',
                r'(?im)^USE \S+;$',
                r';\s*$'
        ):
            self.assertRegexpMatches(output, expected_bit)
