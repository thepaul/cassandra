...
class CqlshCompletionCase(BaseTestCase):
    ...
    def _trycompletions_inner(self, inputstring, immediate='', choices=(), other_choices_ok=False):
        ...
        choicesseen.discard('')
        if other_choices_ok:
            self.assertSetEqual(choicesseen, choicesseen.intersection(choices))
        else:
            self.assertSetEqual(choicesseen, set(choices))
...
class TestCqlshCompletion_CQL2(CqlshCompletionCase):
    ...
    def test_complete_on_empty_string(self):
        self.trycompletions('', choices=('?', 'ALTER', 'ASSUME', 'BEGIN', 'CAPTURE', 'COPY',
                                         'CREATE', 'DEBUG', 'DELETE', 'DESC', 'DESCRIBE', 'DROP',
                                         'HELP', 'INSERT', 'SELECT', 'SHOW', 'SOURCE', 'TRUNCATE',
    ...
    def test_complete_in_string_literals(self):
        ...
        self.trycompletions("create keyspace blah with strategy_class = 'Sim", "pleStrategy'")

    def test_complete_in_numbers(self):
        self.trycompletions("create KEYSPACE foo WITH strategy_class='SimpleStrategy' "
                            "ANd strategy_options:replication_factor = ", choices=('<option_value>',))
